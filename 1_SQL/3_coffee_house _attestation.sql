...
UPDATE employee
SET first_name = 'Кукушкин',
	name = 'Владимир',
	patronymic = 'Константинович',
	phone = '+7(239)917-28-27',
	datebirth = '1984-07-05'
WHERE id = 1;

--Удаление нескольких записей из разных таблиц, по разным критериям (DELETE)

DELETE FROM employee
WHERE id = 5 OR first_name = 'Морозова';

SELECT * FROM post;

DELETE FROM post
WHERE salary = 50000.00 AND name = 'Бухгалтер';
...
SELECT * FROM employee OFFSET 1 LIMIT 1;
...
--Логические операторы (AND,OR), Операторы сравнения (=,!=,<>)

SELECT * FROM employee
WHERE (id = 2 OR id != 4) AND id <> 1;
...
--Объединение таблиц (INNER JOIN, RIGHT JOIN, LEFT JOIN), Создание красивого вывода при помощи псевдонимов (AS);
...
CREATE OR REPLACE VIEW employee_post AS
SELECT 
	employee.id,
	employee.first_name AS "Фамилия",
	employee.name AS "Имя",
